...
class Card::EntropicTest < ActiveSupport::TestCase
  ...
  test "auto_postpone_at uses the period defined in the account by default" do
    ...
    entropies._37s_account.reload.update! auto_postpone_period: 456.days
  ...
  test "setting auto_postpone_period in the board without entropy will create it, without affecting the account entropy" do
    account_entropy = entropies._37s_account
  ...
  test "auto postpone all due using the default account entropy" do
    ...
    cards.logo.update!(last_active_at: 1.day.ago - entropies._37s_account.auto_postpone_period)
    cards.shipping.update!(last_active_at: 1.day.from_now - entropies._37s_account.auto_postpone_period)
    ...
    assert_equal accounts._37s.system_user, cards.logo.postponed_by
  ...
  test "postponing soon scope" do
    [cards.logo, cards.shipping].each(&:published!)
  ...
  test "due_to_be_postponed scope works properly cross-account" do
    ...
    assert_equal([cards.logo, cards.radio].to_set, Card.due_to_be_postponed.to_set)
  end
  ...
  test "postponing_soon scope works properly cross-account" do
    ...
    assert_equal([cards.logo, cards.radio].to_set, Card.postponing_soon.to_set)
  end
end
